...
Each test suite calls a real SDK method (models list, chat completions, embeddings, and more). If the endpoint returns responses the SDK cannot parse, or responses that fail basic validation, the process exits with a non-zero status code.
...
Default suites: `models`, `chat_completions`, `chat_completions_stream`, `embeddings`.
